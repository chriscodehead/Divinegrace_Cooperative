...
if (isset($_POST['sub'])) {
 ...
 $withdrawal_charge = $sqli->getMySavings($saving_id, 'charges');
       ...
       if ($amount_to_withdraw >= $minimum_withdrawal) {
        if ($available_balance > $amount_to_withdraw) {
         ...
         if ($msg = $cal->update($user_tb, $fields, $values, 'email', $sqli->getEmail($_SESSION['user_code']))) {
          ...
          $valuesW = array(null, $trn_id, $sqli->getEmail($_SESSION['user_code']), $amount_to_withdraw, 'processing', $bassic->getDate(), 'Savings Withdrawal', 'no', $plan_type, $coin_type);
          ...
          $subjt = 'Savings Withdrawal Notification From ' . $name;

          $message = 'Hi Admin, ' . $name . ' just made a savings withdrwal of ' . $base_currency . $amount_to_withdraw . '. Payment Details: <br>Bank Name:' . $bank_name . '<br> Account Number: ' . $account_number . '<br> Account Name: ' . $account_name;

          $email_call->generalMessage($subjt, $message, $siteEmail, $siteName, $domain);

          if ($info == 88) {
           $msg = 'Your savings withdrawal was successful expect payment soon!';
          } else {
...
$title = 'Withdraw Savings | ' . $siteName;
$desc = '';
      ...
      <div class="nk-content-body">
       <div class="buysell wide-xs m-auto">
        ...
        <div class="buysell-title text-center">
         <h2 class="title">Withdraw Your Savings!</h2>
        </div>
         ...
         <form action="" method="post" enctype="multipart/form-data" class="buysell-form">
          ...
          <div class="buysell-field form-group">
           ...
           <div class="form-note-group">
            ...
            <span class="buysell-min form-note-alt">Withdrawal Chrage: <?php print $base_currency; ?><?php print $sqli->getMySavings($saving_id, 'charges'); ?> </span>
           </div>
           <div style="margin-top: 20px;" class="form-label-group">
