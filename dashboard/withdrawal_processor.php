	...
	if (isset($_POST['listREF'])) {
		...
		for ($i = 0; $i < count($transactionId); $i++) {
			if ($transactionId[$i] != "") {
				...
				$other = 'Some transaction were not completed!';
			}
		}
		$sum_amount = $sum;
		if ($sum_amount == 0) {
			...
			if ($send == 80) {
				...
				if ($coin_type == 'BTC') {
					...
					$wallet = $cal->selectFrmDB($user_tb, 'account_number', 'email', $sqli->getEmail($_SESSION['user_code']));;
				}
				$email_id = $sqli->getEmail($_SESSION['user_code']);
				$name_id = $cal->selectFrmDB($user_tb, 'first_name', 'email', $sqli->getEmail($_SESSION['user_code']));
				$coin = $coin_type;
				...
				if (empty($wallet)) {
					...
					$trn_id = $bassic->randGenerator();
					$fieldsW = array('id', 'transaction_id', 'email', 'amount', 'status', 'date_time', 'type', 'remove', 'plan_type', 'coin_type');
					$valuesW = array(null, $trn_id, $sqli->getEmail($_SESSION['user_code']), $sum_amount, 'processing', $bassic->getDate(), 'Referral Bonus Withdrawal', 'no', 'REFERRAL', 'BANK');
					$info = $cal->CreatWithdrawBTC($withdraw_tb, $fieldsW, $valuesW);
					if ($info == 88) {
						for ($i = 0; $i < count($transactionId); $i++) {
							if ($transactionId[$i] != "") {
