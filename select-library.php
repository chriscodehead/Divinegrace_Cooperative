			...
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				if ($row['coin_type'] == "BTC") {
					if ($row['btc_address'] == "none") {
						$addressy = 'Network Problem contact: support@omegatrixta.com';
					} else {
						$addressy = @$row['btc_address'];
					}
				} else if ($row['coin_type'] == "ETH") {
					if ($row['ethereum_address'] == "none") {
						$addressy = 'Network Proble contact: support@omegatrixta.com';
					} else {
...
you already made payment. Have any problem? Contact us @ support@omegatrixta.com</td>
								 </tr>'*/;
				} else {
					$add = '';
				}
...
';
	...
	public function GetDepositNEWALL($email)
	{
		...
		if (($stmt)) {
	     ...
	     <tr>
			...
			<td style="font-size:11px;">' . $row['date_created'] . '</td>
			<td style="color:#06C;">' . $row['deposit_status'] . '</td>
	...
	public function createWithdrawalALL($email, $cointype)
	{
		...
		<tr>
		   <!--<td>
			...
			<td >' . $row['date_created'] . '</td>
			<td><a href="withdraw2.php?id=' . $row['coin_type'] . '&ip=' . $row['transaction_id'] . '" class="btn btn-success">Withdraw</a></td>
			<!--' . $cap . '-->
	   </tr> 
	';
				$i++;
	...
	{
		$sql = "SELECT * FROM $this->referral_tb WHERE referral_email = '" . $email . "'   and (`status`='confirmed' || `status`='pending') and `balance` != 0 ORDER BY id DESC";
		$dbs = new DBConnection();
		$db = $dbs->DBConnections();
		$stmt = $db->prepare($sql);
		$stmt->execute();
	...
	public function selectReferralUserDash($email)
	{
		$sql = "SELECT * FROM $this->referral_tb WHERE referral_email = '" . $email . "' ORDER BY id DESC";
		$dbs = new DBConnection();
		$db = $dbs->DBConnections();
		...
		if ($stmt) { //
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				if ($row['pause_status'] == 0) {
					$ps = '<a href="wallet?idd=' . $row['id'] . '&ps=1" title="Pause"> <i class="btn btn-default fa fa-pause btn-success"></i></a>';
				} else {
					$ps = '<a href="wallet?idd=' . $row['id'] . '&ps=0" title="Start" > <i class="btn btn-default fa fa-play btn-primary"></i></a>';
				}
	...
	public function payOutBTC($start, $end)
	{
		...
		if ($stmt) {
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				} else if ($row['coin_type'] == 'YFI') {
					$payT = '<td colspan="13" >User Wallet Address <input style="font-size:20px;" type="text" value="' . self::selectFrmDB($this->user_tb, 'yfi', 'email', $row['email']) . '" class="form-control" id="f-name" name="wallet" placeholder=" "></td>';
				} else {
					$payT = '<td colspan="13" >User Wallet Address <input style="font-size:20px;" type="text" value="' . self::selectFrmDB($this->user_tb, 'wallet_address', 'email', $row['email']) . '" class="form-control" id="f-name" name="wallet" placeholder=" "></td>';
				}
   ...
   <td style="color:#06C;">' . $row['plan_type'] . '</td>-->
	...
	public function SelectAlluser()
	{
		...
		echo '
			<table class="tableclass table-responsive table-bordered table-striped table-condensed" style="font-size:11px;"  width="100%">
						...
						<td style="color:green;">Account Balance</td>
						<td >Date Reg.</td>
						<td >Block</td>
						<td >Edit</td>
						<td >Remove</td>
		...
		if ($stmt) {
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				if ($row['email_activation'] == 'yes') {
					...
					$activation = '<a target="_blank" href ="https://omegatrixta.com/ActivateMail/activate.php?id=' . $row['email'] . '&ip=' . $row['password'] . '">Activate</a>';
				}
			...
			<!--<td style="color:green;"><a href="add-transaction?id=' . $row['email'] . '">
			<i class="btn btn-success btn-small fa fa-pencil"> Create</i></a></td>-->
	...
	public function SelectReferralAdmin()
	{
		...
		<tr style="color:' . $color . '">
			...
			<td >' . $row['deposit_category'] . '</td>
			<td >' . $row['date_created'] . '</td>
	        <td ><a href="edit-ref.php?id=' . $row['id'] . '">Edit</a> 
	        <a data-toggle="modal" href="#myModullDEL' . $row['id'] . '" title="End Transaction"><i class="btn btn-default fa fa-trash-o"></i></a></td>
			</tr>
		    ...
		    <div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="myModullDEL' . $row['id'] . '" class="modal fade">
		              <div class="modal-dialog">
		                  <div class="modal-content">
		                      ...
		                      <div class="modal-footer">
		                          <button data-dismiss="modal" class="btn btn-default" type="button">Cancel</button>
		                          <a href="referral-history.php?del=' . $row['id'] . '">
								  <span class="btn btn-theme">Delete</span>
								  </a>
		                      </div>
		                  </div>
